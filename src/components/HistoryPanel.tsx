...
function entryLabel(stars: number, sessionType?: SessionType): string {
...
function entryDisplayTitle(
  ...
  if (stars < 0) return title ?? bookTitle ?? 'Stars removed';
...
export default function HistoryPanel() {
  ...
  const timeline = useMemo((): TimelineItem[] => {
    const items: TimelineItem[] = [
      ...entries.map(e => ({
        ...
        title: entryDisplayTitle(e.stars, e.title, e.bookTitle, e.sessionType),
        note: e.note,
        stars: e.stars,
      })),
      ...redemptions.map(r => ({
  ...
  const dailyGroups = useMemo(() => {
    ...
    for (const item of timeline) {
      const existing: DayData = map.get(item.date) ?? {
        date: item.date,
        earned: 0,
        deducted: 0,
        redeemed: 0,
  ...
  function amountText(item: TimelineItem): string {
    if (item.kind === 'redemption') return `−${item.stars} ⭐`;
    return `${item.stars > 0 ? '+' : ''}${item.stars} ⭐`;
  }

  function amountStyle(item: TimelineItem): React.CSSProperties | undefined {
    if (item.kind === 'redemption' || (item.kind === 'entry' && item.stars < 0)) {
  ...
  return (
    <div>
          ...
          {timeline.length === 0 ? (
            <div className="empty-state">
              ...
              <p className="empty-state-text">
                Start logging sessions and they'll appear here.
              </p>
            </div>
          ) : (
            <div
                ...
                <div key={item.id} className="history-item">
                  ...
                  <div className="history-body">
                    ...
                    <p className="history-meta">
                      {item.kind === 'entry'
                        ? entryLabel(item.stars, item.sessionType)
                        : '🎁 Redeemed'}{' '}
      ...
      <ConfirmDialog
        ...
        confirmLabel="Undo"
        onConfirm={() => {
          if (deleteConfirm) deleteRedemption(deleteConfirm);
