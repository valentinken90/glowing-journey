...
export default function LogStars({ showToast }: LogStarsProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    if (editingEntry) {
      ...
      addEntry(data);
      showToast(`⭐ ${pluralStars(form.stars)} added!`);
    }
    setModalOpen(false);
  ...
  return (
    <div>
      <div className="screen-header">
        ...
        <p className="screen-subtitle">Record every reading session</p>
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="empty-state-text">Tap the button above to log your first reading session.</p>
        </div>
      ) : (
              ...
              <div className="entry-item-header">
                <div className="entry-item-main">
                  <p className="entry-item-title">
                    {entry.bookTitle ?? 'Reading session'}
                  </p>
                  <p className="entry-item-meta">{formatDate(entry.date)}</p>
                </div>
      ...
      <Modal
        ...
        title={editingEntry ? 'Edit Entry' : 'Add Reading Session'}
      >
        <div className="stack stack-lg" style={{ marginBottom: 20 }}>
          ...
          <div className="form-field">
            ...
            <div className="star-picker">
              <button
                ...
                onClick={() => setForm(f => ({ ...f, stars: clamp(f.stars - 1, 1, 50) }))}
              >
                −
              </button>
          ...
          {/* Book title */}
          <div className="form-field">
            <label className="form-label" htmlFor="entry-book">Book Title (optional)</label>
            <input
              id="entry-book"
              type="text"
              className="form-input"
          ...
          <div className="form-field">
            ...
            <textarea
              ...
              placeholder="e.g. Read two whole chapters!"
              value={form.note}
      ...
      <ConfirmDialog
        ...
        title="Delete Entry?"
        message={`This will remove "${deleteConfirm?.bookTitle ?? 'this reading session'}" and adjust the star balance accordingly.`}
        confirmLabel="Delete"
        confirmDanger
